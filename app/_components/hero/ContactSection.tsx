  ...
  return (
    ...
    >
      ...
      >
        <motion.h3
          variants={scaleIn}
          className="text-3xl font-bold text-indigo-600 dark:text-gray-600 mb-2 text-center drop-shadow-lg"
        >
          {t.contact.title}
        </motion.h3>
        <motion.p
          variants={fadeIn}
          className="mb-6 text-sm opacity-80 text-center"
        >
          {t.contact.desc}{" "}
          <a
        ...
        >
          <motion.input
            ...
            value={form.name}
            onChange={(e) =>
              setForm((f: FormData) => ({ ...f, name: e.target.value }))
            }
            required
          />
          <motion.input
            ...
            value={form.email}
            onChange={(e) =>
              setForm((f: FormData) => ({ ...f, email: e.target.value }))
            }
            type="email"
            required
          />
          <motion.textarea
            ...
            placeholder={
              lang === "pl" ? "Opisz swój projekt" : "Describe your project"
            }
            value={form.message}
            onChange={(e) =>
              setForm((f: FormData) => ({ ...f, message: e.target.value }))
            }
            required
          />
          <motion.button
